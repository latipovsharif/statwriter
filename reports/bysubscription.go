...
func subscriptionStat(db *sqlx.DB) ([]BaseStr, error) {
	...
	err := db.Select(&s, `SELECT group_field, req_count, resp_count, buy_count, click_count,
                ...
                FROM (
	                SELECT dateDiff('day', toDate(extract(visitParamExtractRaw(req_user_ext, 'subscriptionDateTime'), '(\\d{4}-\\d{2}-\\d{2})')), toDate(now())) AS group_field , count(*) as req_count
                    FROM bid_requests
                ...
                ALL LEFT JOIN (
                    ...
                    FROM (
                        SELECT dateDiff('day', toDate(extract(visitParamExtractRaw(req_user_ext, 'subscriptionDateTime'), '(\\d{4}-\\d{2}-\\d{2})')), toDate(now())) AS group_field, count(*) as resp_count
                        FROM bid_responses
                    ...
                    ALL LEFT JOIN (
    	                ...
    	                FROM (
        	                SELECT dateDiff('day', toDate(extract(visitParamExtractRaw(req_user_ext, 'subscriptionDateTime'), '(\\d{4}-\\d{2}-\\d{2})')), toDate(now())) AS group_field, count(*) as buy_count, sum(buyout_price) as buyout_sum, sum(resp_seat_bid_bid_price) as first_sum
                            FROM buyouts
                            WHERE req_site_name != '' AND req_ssp_id IN (2) AND toDate(buyout_date_time) = toDate(now())
                            GROUP BY group_field
                        )
	                    ALL LEFT JOIN (
	 		                SELECT dateDiff('day', toDate(extract(visitParamExtractRaw(req_user_ext, 'subscriptionDateTime'), '(\\d{4}-\\d{2}-\\d{2})')), toDate(now())) AS group_field, count(*) as click_count, sum(click_price) as click_sum
	      	                FROM clicks
	                        WHERE req_site_name != '' AND req_ssp_id IN (2) AND toDate(buyout_date_time) = toDate(now())
	                        GROUP BY group_field
	                    )
